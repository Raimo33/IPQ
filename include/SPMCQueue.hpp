...
class SharedData : public SharedMemoryBase<SharedData<Item, Capacity>>
{
  ...
  public:
    ...
    bool pop(Item &out) noexcept
    {
      ...
      while (true)
      {
        ...
        {
          cached_write_idx = _data.write_idx.load(std::memory_order_acquire);
          if (local_read_idx == cached_write_idx) [[unlikely]]
            return false;
        }
        ...
        {
          out = _data.buffer[local_read_idx];
          return true;
        }
      }
